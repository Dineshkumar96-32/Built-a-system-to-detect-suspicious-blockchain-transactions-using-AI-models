...
# ─────────────────────────────────────────────────────────────
# Fixtures — ALL provided by tests/conftest.py
# ─────────────────────────────────────────────────────────────
# `db` is injected automatically from conftest.py which uses StaticPool:
#
#   poolclass=StaticPool  →  every connection hits the SAME sqlite3 object
#
# This means tables created once stay visible to all fixtures and tests.
# The old local fixtures caused:
#   sqlite3.OperationalError: no such table: transactions  (×11 failures)
# because each new connection got a fresh empty :memory: database.
# ─────────────────────────────────────────────────────────────
